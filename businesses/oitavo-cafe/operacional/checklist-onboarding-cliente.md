# Checklist de Onboarding - Oitavo Café

**Versão:** 1.0.0
**Data:** 2025-12-03
**Objetivo:** Garantir que todo novo cliente tenha uma experiência de início profissional e organizada

---

## Visão Geral do Processo

```
FECHAMENTO → PRÉ-ONBOARDING → KICKOFF → SETUP → INÍCIO
  (Dia 0)      (Dias 1-3)      (Dia 4-5)  (Dias 5-10)  (Dia 10+)
```

**Tempo total:** 7-10 dias úteis do fechamento até início da execução

---

# FASE 1: PRÉ-ONBOARDING (Dias 1-3)

## Após Fechamento (Dia 0-1)

### Administrativo
- [ ] Contrato assinado e arquivado
- [ ] Primeiro pagamento confirmado
- [ ] Cliente cadastrado no sistema/planilha de clientes
- [ ] Pasta do cliente criada (Google Drive/Notion/etc)

### Comunicação Inicial
- [ ] Enviar mensagem de boas-vindas (WhatsApp)
- [ ] Enviar email de boas-vindas com próximos passos
- [ ] Agendar reunião de Kickoff (1-2h)

### Mensagem de Boas-Vindas (WhatsApp)

```
Oi [NOME]! 🎉

Seja muito bem-vinda à Oitavo Café!

Estou muito feliz em começar essa parceria. Agora vamos transformar o marketing do [NEGÓCIO] em uma máquina de vendas!

Próximos passos:
1. Vou te enviar um email com algumas informações que preciso de você
2. Vamos agendar nossa reunião de Kickoff para [DATA/HORA]
3. A partir daí, começamos a trabalhar!

Qualquer dúvida, me chama aqui.

Vamos juntos! 💪
```

### Email de Boas-Vindas

```
Assunto: Bem-vinda à Oitavo Café! Próximos passos 🚀

Oi [NOME],

Seja muito bem-vinda à Oitavo Café!

Estou animado em começar essa parceria e transformar o marketing do [NEGÓCIO].

PRÓXIMOS PASSOS:

1. REUNIÃO DE KICKOFF
   Data: [DATA]
   Horário: [HORA]
   Duração: 1-2 horas
   Link: [LINK DA CALL]

2. ANTES DA REUNIÃO, PRECISO QUE VOCÊ:
   (responda este email com as informações abaixo)

   □ Acesso ao Instagram (login + senha ou convite como admin)
   □ Acesso ao Facebook (se tiver página)
   □ Logo em alta qualidade (PNG ou AI)
   □ Cores da marca (códigos hex se tiver)
   □ Fontes da marca (se tiver)
   □ Fotos do negócio (mínimo 10-20 fotos boas)
   □ Fotos suas (para posts que você aparece)
   □ 3-5 concorrentes que você admira ou quer superar
   □ O que você NÃO quer que a gente faça (algo que não combina com a marca)

3. NA REUNIÃO DE KICKOFF VAMOS:
   - Alinhar estratégia e posicionamento
   - Definir pilares de conteúdo
   - Criar calendário editorial do primeiro mês
   - Tirar todas as dúvidas
   - Definir rotina de trabalho

Se tiver qualquer dúvida antes da reunião, me chama no WhatsApp.

Vamos juntos!

[SEU NOME]
Oitavo Café
```

---

## Preparação para Kickoff (Dias 2-3)

### Coletar da Cliente (via email/formulário)
- [ ] Acesso ao Instagram (admin)
- [ ] Acesso ao Facebook (se tiver)
- [ ] Logo em alta qualidade
- [ ] Cores da marca
- [ ] Fontes (se tiver)
- [ ] Fotos do negócio (10-20)
- [ ] Fotos da cliente
- [ ] Lista de concorrentes
- [ ] O que NÃO fazer

### Preparação Interna
- [ ] Analisar Instagram atual (posts, stories, bio, destaques)
- [ ] Analisar concorrentes
- [ ] Rascunhar posicionamento e pilares de conteúdo
- [ ] Preparar apresentação/agenda do Kickoff

---

# FASE 2: REUNIÃO DE KICKOFF (Dia 4-5)

## Agenda da Reunião (1-2 horas)

### Parte 1: Alinhamento (20 min)
- [ ] Apresentar como vai funcionar o trabalho
- [ ] Explicar divisão de responsabilidades
- [ ] Definir canais de comunicação
- [ ] Definir frequência de reuniões (mensal)

### Parte 2: Estratégia (30 min)
- [ ] Revisar/definir posicionamento
- [ ] Definir tom de voz
- [ ] Definir 3-5 pilares de conteúdo
- [ ] Alinhar público-alvo

### Parte 3: Conteúdo (30 min)
- [ ] Apresentar calendário editorial do mês 1
- [ ] Explicar tipos de post (feed, carrossel, reels, stories)
- [ ] Definir o que ela precisa fazer (stories, vídeos)
- [ ] Alinhar frequência (1-2 posts/dia + stories diários)

### Parte 4: Operacional (20 min)
- [ ] Definir processo de aprovação (se houver)
- [ ] Definir prazo de entrega dos posts
- [ ] Definir como ela envia conteúdo bruto
- [ ] Agendar próxima reunião (mensal)

### Parte 5: Dúvidas (20 min)
- [ ] Responder todas as dúvidas
- [ ] Confirmar próximos passos
- [ ] Confirmar que ela tem tudo que precisa

## Checklist Pós-Kickoff
- [ ] Enviar resumo da reunião por email
- [ ] Enviar calendário editorial do mês 1
- [ ] Confirmar acesso a todas as contas
- [ ] Agendar reunião mensal recorrente

### Email Pós-Kickoff

```
Assunto: Resumo do Kickoff + Próximos Passos

Oi [NOME],

Foi ótimo nosso kickoff! Aqui está o resumo do que alinhamos:

ESTRATÉGIA DEFINIDA:
- Posicionamento: [RESUMO]
- Tom de voz: [RESUMO]
- Pilares de conteúdo:
  1. [PILAR 1]
  2. [PILAR 2]
  3. [PILAR 3]

DIVISÃO DE RESPONSABILIDADES:
Oitavo Café:
- [X] posts/mês profissionais
- Calendário editorial diário
- Direção criativa
- Análise mensal

Você:
- Stories diários
- Posts complementares seguindo calendário
- Gravação de vídeos/reels
- Interação com seguidores

CALENDÁRIO DO MÊS 1:
[Anexar ou linkar o calendário]

PRÓXIMOS PASSOS:
1. Vou começar a criar os primeiros posts
2. Te envio para aprovação até [DATA]
3. Começamos a postar em [DATA]
4. Próxima reunião: [DATA]

Dúvidas? Me chama no WhatsApp.

Vamos! 🚀

[SEU NOME]
```

---

# FASE 3: SETUP (Dias 5-10)

## Setup de Contas

### Instagram
- [ ] Verificar se é conta Business/Creator
- [ ] Otimizar bio (se necessário)
- [ ] Organizar destaques (se necessário)
- [ ] Configurar link na bio (Linktree/similar)
- [ ] Verificar conexão com Facebook

### Facebook (se aplicável)
- [ ] Verificar página configurada corretamente
- [ ] Conectar Instagram à página
- [ ] Configurar Business Manager (se tráfego pago)

### Tráfego Pago (se aplicável)
- [ ] Criar/acessar Business Manager
- [ ] Configurar pixel no site
- [ ] Criar públicos iniciais
- [ ] Configurar formas de pagamento

### Ferramentas Internas
- [ ] Criar board/pasta para o cliente
- [ ] Configurar ferramenta de agendamento
- [ ] Criar templates de arte base
- [ ] Configurar calendário de conteúdo

## Criação Inicial

### Primeiros Posts
- [ ] Criar primeiros [X] posts do mês
- [ ] Enviar para aprovação da cliente (se aplicável)
- [ ] Ajustar conforme feedback
- [ ] Agendar posts

### Calendário Editorial
- [ ] Criar calendário completo do mês 1
- [ ] Incluir ideias para posts da cliente
- [ ] Incluir ideias de stories
- [ ] Enviar para cliente

---

# FASE 4: INÍCIO DA EXECUÇÃO (Dia 10+)

## Primeira Semana de Execução

### Dia 1-2
- [ ] Postar primeiros posts
- [ ] Confirmar que cliente está fazendo stories
- [ ] Verificar se calendário está claro

### Dia 3-5
- [ ] Check-in rápido com cliente (WhatsApp)
- [ ] Ajustar qualquer dúvida
- [ ] Monitorar primeiros resultados

### Fim da Semana 1
- [ ] Verificar se frequência está sendo mantida
- [ ] Coletar feedback inicial da cliente
- [ ] Ajustar o que for necessário

## Mensagem de Check-in (Semana 1)

```
Oi [NOME]!

Primeira semana juntos! 🎉

Como está sendo? Conseguiu fazer os stories seguindo o calendário?

Alguma dúvida ou dificuldade?

Me conta como está se sentindo com o novo processo.
```

---

# ROTINA MENSAL (Após Onboarding)

## Semana 1 do Mês
- [ ] Criar calendário editorial do mês
- [ ] Criar posts da primeira quinzena
- [ ] Enviar calendário para cliente

## Semana 2 do Mês
- [ ] Criar posts da segunda quinzena
- [ ] Check-in rápido com cliente
- [ ] Ajustes se necessário

## Semana 3 do Mês
- [ ] Monitorar resultados
- [ ] Preparar análise mensal
- [ ] Identificar o que está funcionando

## Semana 4 do Mês
- [ ] Reunião mensal de alinhamento
- [ ] Apresentar resultados
- [ ] Planejar próximo mês
- [ ] Coletar feedback

## Reunião Mensal - Agenda (30-45 min)

1. **Resultados do mês** (10 min)
   - Métricas principais
   - O que funcionou
   - O que ajustar

2. **Feedback da cliente** (10 min)
   - Como ela está se sentindo
   - Dificuldades
   - Sugestões

3. **Planejamento próximo mês** (10 min)
   - Temas/campanhas
   - Datas importantes
   - Ajustes na estratégia

4. **Próximos passos** (5 min)
   - Confirmar ações
   - Agendar próxima reunião

---

# DOCUMENTOS DO CLIENTE

## Pasta do Cliente (Google Drive/Notion)

```
📁 [NOME DO CLIENTE]
├── 📁 01_Contrato
│   └── Contrato assinado.pdf
├── 📁 02_Briefing
│   ├── Informações do negócio.md
│   ├── Acessos.md (criptografado)
│   └── Brand assets/
├── 📁 03_Estratégia
│   ├── Posicionamento.md
│   ├── Pilares de conteúdo.md
│   └── Tom de voz.md
├── 📁 04_Calendários
│   ├── 2025-01_Calendário.xlsx
│   ├── 2025-02_Calendário.xlsx
│   └── ...
├── 📁 05_Conteúdo
│   ├── Posts/
│   ├── Stories/
│   └── Reels/
├── 📁 06_Relatórios
│   ├── 2025-01_Relatório.pdf
│   └── ...
└── 📁 07_Reuniões
    ├── Kickoff_notas.md
    └── Mensal_notas/
```

---

# MÉTRICAS DE SUCESSO DO ONBOARDING

## Cliente Bem Onboardado

- [ ] Entende como funciona o modelo de trabalho
- [ ] Sabe exatamente o que precisa fazer
- [ ] Tem acesso ao calendário editorial
- [ ] Está fazendo stories diários
- [ ] Está complementando posts conforme calendário
- [ ] Sabe como se comunicar com a Oitavo Café
- [ ] Não tem dúvidas pendentes

## Red Flags no Onboarding

⚠️ **Atenção se:**
- Cliente não responde mensagens em 48h+
- Cliente não enviou os acessos/materiais
- Cliente não está fazendo a parte dela
- Cliente reclama de não entender o processo

**Ação:** Conversa direta para realinhar expectativas

---

# METADADOS

**Versão:** 1.0.0
**Criado em:** 2025-12-03
**Baseado em:** Modelo de trabalho Oitavo Café
